...
public class SwaggerDemoController {
    ...
    public Object set(
            @ApiParam(value = "这是入参id", name = "这是param's name", defaultValue = "1") @PathVariable Long id,
            @ApiParam(value = "入参新名字", defaultValue = "默认名字") @RequestParam(required = false) String newName,
            @ApiParam(value = "入参哈哈哈", required = true, defaultValue = "默认无") @RequestParam(required = false, defaultValue = "haha") String slogon,
            @ApiParam(value = "入参times", defaultValue = "20") @RequestParam(required = false, defaultValue = "11") Long times) {
